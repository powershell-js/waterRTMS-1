<template>
    <div class="outbox" :style="{height:listoutboxHeight+'px'}">
         <div class="wrsybox"> 
           <div class="leftbox"> 
                <div class="line"></div>
                <div class="linerighttext">污染扩散分析结果</div>
           </div>
           <div class="rightbox">
               <el-button v-on:click="Diffusion"  style="margin-right:20px;font-size:14px;" type="success" size="min">扩散分析</el-button>
           </div>
        </div>
             <div class="syfwtext">污染扩散范围内用户列表</div>
...
export default {
  ...
  methods:{
    myEventListener:function(event){
              ...
              if(typeTemp=="DiffusionReturn"){
                  this.tableData = dataTemp;
              }
         },
    Diffusion(){
      WindowsEvent.MyDispatchEvent("Diffusion",'');
